...
def get_profit_entity_data(type):
    exited_shares = Stock.objects.filter(sell_date__isnull=False)
    exited_shares_serializer = ExitedStockSerializer(exited_shares, many=True)
    if type == "lowtohigh":
        return {"exited_share": sort_by_entity(exited_shares_serializer.data, "profit", "lh")}
    elif type == "hightolow":
        return {"exited_share": sort_by_entity(exited_shares_serializer.data, "profit", "hl")}
    return None


def get_loss_entity_data(type):
    exited_shares = Stock.objects.filter(sell_date__isnull=False)
    exited_shares_serializer = ExitedStockSerializer(exited_shares, many=True)
    if type == "lowtohigh":
        return {"exited_share": sort_by_entity(exited_shares_serializer.data, "loss", "lh")}
    elif type == "hightolow":
        return {"exited_share": sort_by_entity(exited_shares_serializer.data, "loss", "hl")}
    return None


def get_units_entity_data(type):
    exited_shares = Stock.objects.filter(sell_date__isnull=False)
    exited_shares_serializer = ExitedStockSerializer(exited_shares, many=True)
    non_exited_share = Stock.objects.filter(sell_date__isnull=True)
    non_exited_share_serializer = NonExitedStockSerializer(non_exited_share, many=True)
...
def get_sorting_data(entity, type):
    if entity == "profit":
        if type == "hightolow" or type == "lowtohigh":
            return get_profit_entity_data(type)
        return None
    elif entity == "loss":
        if type == "hightolow" or type == "lowtohigh":
            return get_loss_entity_data(type)
        return None
    elif entity == "units":
        if type == "hightolow" or type == "lowtohigh":
            return get_units_entity_data(type)
        return None
    return None
